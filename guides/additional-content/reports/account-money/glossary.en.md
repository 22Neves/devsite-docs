...
| Data type | This column provides detailed information about the data type associated with each field, including the maximum length and the corresponding format. Example: `STRING(200)` (for text strings with a maximum of 200 characters),` NUMERIC(19,2)` (for numbers with 19 digits, including 2 decimal places), and `STRING(JSON)` (for structured data in JSON format). |
