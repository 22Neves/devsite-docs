...
|---|---|---|
| Reference code (`EXTERNAL_REFERENCE`) |  ID that helps identify the origin of the transaction. For example, it can be the sale through the order ID or the shipment (if it is a cart purchase) or the ID itself provided by the seller in case of an external integration. <br><br> Please note that this field might be empty in some cases such as the invoice payment, money transfer etc. <br>  | String <br> (200) |
| Mercado Pago transaction ID (`SOURCE_ID`) | Transaction ID in Mercado Pago (e.g an order payment). This field contains alphaNumeric <br> (19,2) values. | Numeric <br> (19,2) |
| Seller account code (`USER_ID`) | Seller account code. (Cust ID). | Numeric <br> (19,2) |
| Payment method (`PAYMENT_METHOD`) | Check the [available payment methods](/developers/en/docs/sales-processing/payment-methods) according to the country you operate with Mercado Pago. | String <br> (200) |
| Payment method type (`PAYMENT_METHOD_TYPE`) | Payment method type. It can be: <br><br> *credit_card*: credit card.<br>*debit_card*: debit card.<br>*bank_transfer*: transfer.<br>*atm*: ATM. <br>*ticket*: cash.<br>*account_money*: account money. <br> | String <br> (200) |
...
| Purchase amount (`TRANSACTION_AMOUNT`) | Transaction net amount. | Numeric <br> (19,2) |
| Currency (`TRANSACTION_CURRENCY`) | Can assume some of these values as appropriate: <br><br> MXN (Peso Mexicano) <br> CLP (Peso Chileno) <br> ARS (Peso Argentino) <br> BRL (Real Brasilero) <br> COP (Peso Colombiano) <br> PEN (Sol Peruano) <br> UYU (Peso Uruguayo) <br> VES (Bolivar Venezolano) <br> | String <br> (200) |
| Amount received for split purchases (`SELLER_AMOUNT`) | Amount received for split purchases. | Numeric <br> (19,2) |
| Date of origin (`TRANSACTION_DATE`) | Transaction creation date. | Numeric <br> (19,2) |
| Fees + VAT  (`FEE_AMOUNT`) | Sum of the processing, shipping, installments and coupon fees if it was at seller's expense. Includes VAT. | Numeric <br> (19,2) |
| Net amount of the transaction that impacted your balance (`SETTLEMENT_NET_AMOUNT`) | Net amount of the transaction that impacted the balance. All fees involved were deducted from the purchase amount `TRANSACTION_AMOUNT`. |  Numeric <br> (19,2) |
| Settlement currency (`SETTLEMENT_CURRENCY`) | Can assume some of these values as appropriate: <br><br> MXN (Peso Mexicano) <br> CLP (Peso Chileno) <br> ARS (Peso Argentino) <br> BRL (Real Brasilero) <br> COP (Peso Colombiano) <br> PEN (Sol Peruano) <br> UYU (Peso Uruguayo) <br> VES (Bolivar Venezolano) <br> | String <br> (200) |
| Approval date (`SETTLEMENT_DATE`) | Transaction approval date. | DateTime <br> (yyyy-MM-dd'T'HH:mm:ssZ) |
| Transaction net amount (`REAL_AMOUNT`) | Net amount of the transaction, if it is a approved payment (settlement), the amounts regarding chargebacks, complaints or returns are discounted. | Numeric <br> (19,2) |
| Discount coupon (`COUPON_AMOUNT`) | Amount of the discount coupon. **It is only deducted from gross amount or purchase amount** (`TRANSACTION_AMOUNT`) **if provided by the seller.** | Numeric <br> (19,2) |
...
| Mercado Libre feed + VAT (`MKP_FEE_AMOUNT`) | Mercado Libre Fee. Includes VAT. | Numeric <br> (19,2) |
| Fee for offering interest-free installments (`FINANCING_FEE_AMOUNT`) | Fee for offering interest-free installments. | Numeric <br> (19,2) |
| Shipping cost (`SHIPPING_FEE_AMOUNT`) | Shipping cost. | Numeric <br> (19,2) |
| Taxes collected for IIBB withholdings (`TAXES_AMOUNT`) | Tax collected for withholdings of Gross Income, VAT, Profits; and taxes on Credits and Debits, among others. [Check further details about withholdings.](https://vendedores.mercadolibre.com.ar/nota/retenciones-y-percepciones-sobre-tus-ventas-lo-que-debes-saber/) | Numeric <br> (19,2) |
| Installments (`INSTALLMENTS`) | Number of installments in which the transaction was carried out. | Numeric <br> (19,2) |
| Tax details (`TAX_DETAIL`) | Description of the tax withheld for transaction in the taxes collected by IIBB withholdings `TAXES_AMOUNT`.<br><br> It can assume the following values according to the jurisdiction: <br>cordoba<br>corrientes<br>mendoza<br>la_pampa<br>santa_fe<br>tucuman<br>entre_rios<br>catamarca<br>neuquen<br>santiago_del_estero<br>rio_negro<br>jujuy  | String <br> (200) |
| Cashier ID (`POS_ID`) | Cashier ID if the payment is made at a physical store. | String <br> (200) |
| Cashier name (`POS_NAME`) | Name of the cashier for the payment made at a physical store. | String <br> (200) |
| Cashier ID defined by the user (`EXTERNAL_POS_ID`) | Cashier ID defined by the user for the payment made at a physical store. | String <br> (200) |
| Store ID (`STORE_ID`) | Store ID if the payment is made at a physical store. | String <br> (200) |
| Store name (`STORE_NAME`) | Store name if the payment is made at a physical store. | String <br> (200) |
| Store ID defined by the user (`EXTERNAL_STORE_ID`) | Store ID defined by the user for the payment made at a physical store. | String <br> (200) |
| Order ID (`ORDER_ID`) | Purchase Order. | Numeric <br> (19,2) |
| Shipping ID (`SHIPPING_ID`) | Shipping Identification. | Numeric <br> (19,2) |
| Shipping method (`SHIPMENT_MODE`) | Shipping Method. | String <br> (200) |
| Package ID (`PACK_ID`) | Package ID in the cart. | Numeric <br> (19,2) |
| Detailed taxes (`TAXES_DISAGGREGATED`) | Detailed taxes in the JSON format. | String <br> (JSON) |
| POS machine series number (S/N) (`POI_ID`) | POS machine ID if the payment is made at a physical store. | String <br> (200) |
...
| Description (`DESCRIPTION`) | It helps identify the register of the transactions or operations in a period of time.<br> When its a payment in installments, it will be "INSTALLMENT". | String <br> (200) | 
...
| Buyer card (`CARD_INITIAL_NUMBER`) | Corresponds to the first digits of the credit or debit card used in a purchase. | Numeric <br> (19,2) |
...
| Payer name (`PAYER_NAME`) | Name of the person who makes a payment or a donation. | String <br> (200) |
| Payer ID type (`PAYER_ID_TYPE`) | Type of identification of the person who makes a payment or a donation. | String <br> (200) |
| Payer ID number (`PAYER_ID_NUMBER`) | Identification number of the person who makes a payment or a donation. | String <br> (200) | 
| Sales channel (`BUSINESS_UNIT`) | Corresponds to the channel through which an order was generated. The channels are Mercado Pago, Mercado Libre, Mercado Shops and Delivery. | String <br> (200) |
| Payment platform (`SUB_UNIT`) | It allows to identify the method used to collect a payment with Mercado Pago. | String <br> (200) |
...
| Amount received for split purchases (`SELLER_AMOUNT`) | Amount received for split purchases. | Numeric <br> (19,2) |
| Date of origin (`TRANSACTION_DATE`) | Transaction creation date. | Numeric <br> (19,2) |
| Fees (`FEE_AMOUNT`) | Sum of the processing, shipping, installments and coupon fees if it was at seller's expense. | Numeric <br> (19,2) |
| Net amount of the transaction that impacted your balance (`SETTLEMENT_NET_AMOUNT`) | Net amount of the transaction that impacted the balance. All fees involved were deducted from the purchase amount `TRANSACTION_AMOUNT`. |  Numeric <br> (19,2) |
| Settlement currency (`SETTLEMENT_CURRENCY`) | Can assume some of these values as appropriate: <br><br> MXN (Peso Mexicano) <br> CLP (Peso Chileno) <br> ARS (Peso Argentino) <br> BRL (Real Brasilero) <br> COP (Peso Colombiano) <br> PEN (Sol Peruano) <br> UYU (Peso Uruguayo) <br> VES (Bolivar Venezolano) <br> | String <br> (200) |
| Approval date (`SETTLEMENT_DATE`) | Transaction approval date. | DateTime <br> (yyyy-MM-dd'T'HH:mm:ssZ) |
| Transaction net amount (`REAL_AMOUNT`) | Net amount of the transaction, if it is a approved payment (settlement), the amounts regarding chargebacks, complaints or returns are discounted. | Numeric <br> (19,2) |
...
| Mercado Libre feed (`MKP_FEE_AMOUNT`) | Mercado Libre Fee. | Numeric <br> (19,2) |
| Fee for offering interest-free installments (`FINANCING_FEE_AMOUNT`) | Fee for offering interest-free installments. | Numeric <br> (19,2) |
| Shipping cost (`SHIPPING_FEE_AMOUNT`) | Shipping cost. | Numeric <br> (19,2) |
| Taxes collected for withholdings (`TAXES_AMOUNT`) | Taxes collected. | Numeric <br> (19,2) |
| Installments (`INSTALLMENTS`) | Number of installments in which the transaction was carried out. | Numeric <br> (19,2) |
| Tax details (`TAX_DETAIL`) | Description of the tax withheld for transaction in the taxes `TAXES_AMOUNT`. | String <br> (200) |
| Cashier ID (`POS_ID`) | Cashier ID if the payment is made at a physical store. | String <br> (200) |
| Cashier name (`POS_NAME`) | Name of the cashier for the payment made at a physical store. | String <br> (200) |
| Cashier ID defined by the user (`EXTERNAL_POS_ID`) | Cashier ID defined by the user for the payment made at a physical store. | String <br> (200) |
| Store ID (`STORE_ID`) | Store ID if the payment is made at a physical store. | String <br> (200) |
...
| Fee for offering interest-free installments (`FINANCING_FEE_AMOUNT`) | Fee for offering interest-free installments. | Numeric <br> (19,2) |
| Shipping cost (`SHIPPING_FEE_AMOUNT`) | Shipping cost. | Numeric <br> (19,2) |
| Taxes collected for IIBB withholdings (`TAXES_AMOUNT`) | Impuestos cobrados por retenciones de IVA. Taxes collected for withholdings of VAT, ITT and source according to what is applicable.| Numeric <br> (19,2) |
| Installments (`INSTALLMENTS`) | Number of installments in which the transaction was carried out. | Numeric <br> (19,2) |
| `TAX_AMOUNT_TELCO` | Description of the taxes collected by operation in the `TAXES_AMOUNT`. It can be:<br><br> fuente<br>iva<br>ica<br> | Numeric <br> (19,2) |
| Tax details (`TAX_DETAIL`) | Description of the tax withheld for transaction in the taxes collected by IIBB withholdings `TAXES_AMOUNT`. | String <br> (200) |
| Cashier ID (`POS_ID`) | Cashier ID if the payment is made at a physical store. | String <br> (200) |
| Cashier name (`POS_NAME`) | Name of the cashier for the payment made at a physical store. | String <br> (200) |
| Cashier ID defined by the user (`EXTERNAL_POS_ID`) | Cashier ID defined by the user for the payment made at a physical store. | String <br> (200) |
| Store ID (`STORE_ID`) | Store ID if the payment is made at a physical store. | String <br> (200) |
...
| Fee for offering interest-free installments (`FINANCING_FEE_AMOUNT`) | Fee for offering interest-free installments. | Numeric <br> (19,2) |
| Shipping cost (`SHIPPING_FEE_AMOUNT`) | Shipping cost. | Numeric <br> (19,2) |
| Taxes collected for IIBB withholdings (`TAXES_AMOUNT`) | Impuestos cobrados por retenciones de IVA.| Numeric <br> (19,2) |
| Installments (`INSTALLMENTS`) | Number of installments in which the transaction was carried out. | Numeric <br> (19,2) |
| `TAX_AMOUNT_TELCO` | The value of the tax on telecommunications companies that is deducted from the gross value.  | Numeric <br> (19,2) |
| Tax details (`TAX_DETAIL`) | Description of the tax withheld for transaction in the taxes collected by IIBB withholdings `TAXES_AMOUNT`. | String <br> (200) |
| Cashier ID (`POS_ID`) | Cashier ID if the payment is made at a physical store. | String <br> (200) |
| Cashier name (`POS_NAME`) | Name of the cashier for the payment made at a physical store. | String <br> (200) |
| Cashier ID defined by the user (`EXTERNAL_POS_ID`) | Cashier ID defined by the user for the payment made at a physical store. | String <br> (200) |
...
| Fee for offering interest-free installments (`FINANCING_FEE_AMOUNT`) | Fee for offering interest-free installments. | Numeric <br> (19,2) |
| Shipping cost (`SHIPPING_FEE_AMOUNT`) | Shipping cost. | Numeric <br> (19,2) |
| Taxes collected for IIBB withholdings (`TAXES_AMOUNT`) | Taxes collected | Numeric <br> (19,2) |
| Installments (`INSTALLMENTS`) | Number of installments in which the transaction was carried out. | Numeric <br> (19,2) |
| Tax details (`TAX_DETAIL`) | Description of the tax withheld for transaction in the taxes collected by IIBB withholdings `TAXES_AMOUNT`. | String <br> (200) |
| Cashier ID (`POS_ID`) | Cashier ID if the payment is made at a physical store. | String <br> (200) |
| Cashier name (`POS_NAME`) | Name of the cashier for the payment made at a physical store. | String <br> (200) |
