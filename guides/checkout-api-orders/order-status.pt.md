...
| `charged_back` | `in_process` | A order sofreu uma contestação. Isso significa que uma das transações da ordem foi contestada e está em processo de avaliação. |
| `charged_back` | `settled` | A order sofreu uma contestação. Isso significa que a transação foi liquidada. Isso pode ocorrer quando o valor da transação foi processado e confirmado. |
| `charged_back` | `reimbursed` | A order sofreu uma contestação. Isso significa que a transação foi reembolsada e o valor da transação foi devolvido ao pagador após o estorno.  |
