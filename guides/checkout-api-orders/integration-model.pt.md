...
| --- | --- |--- |
| Processamento do pagamento  | Automático (crie e processe sua transação) | [Automático ou manual](/developers/ew/docs/checkout-api/integration-model#bookmark_modos_de_processamento_de_orders) (escolhendo quando processar a  sua transação) . |
...
- [**Reembolsar order**](/developers/pt/reference/orders/online-payments/refund/post): no caso do modo automático, podem ser criados estornos totais ou parciais de um pagamento. A order será reembolsada totalmente se todas as transações forem estornadas por completo. 
...
- **[Alterar](/developers/pt/reference/orders/online-payments/update-transaction/put) e/ou [remover](/developers/pt/reference/orders/online-payments/delete-transaction/delete) transação**: a alteração e remoção de transações só pode ser feita no modo manual e permitem mudar informações de pagamento que já tinham sido adicionadas anteriormente à order. São operações que modificam um item dentro de qualquer campo do parâmetro `transactions`.
...
- [**Processar transação**](/developers/pt/reference/orders/online/process-order/post): possibilitada a execução das transações criadas e/ou alteradas no modo manual. 
- [**Obter order**](/developers/pt/reference/orders/online-payments/get-order/get): permite obter informações sobre uma order, incluindo o seu status em tempo real.
- [**Cancelar order**](/developers/pt/reference/orders/online-payments/cancel-order/post): responsável pelo cancelamento de um order já existente, mas não que ainda não foi processada. 
