...
Para realizar uma autorização de reserva de valores, envie um **POST** com seu *Access Token de teste* e todos os atributos necessários ao endpoint [/v1/orders :TagComponent{textTag="API"}](/developers/pt/reference/order/online-payments/create/post), Será necessário definir o campo `capture_mode` como `manual` para posteriormente capturar os fundos que você reserva.
...
Para realizar a captura do valor total de uma reserva, é necessário enviar uma requisição ao endpoint [/v1/orders/{order_id}/capture :TagComponent{textTag="API"}](/developers/pt/reference/order/online-payments/capture/post) com seu *Access Token de teste*, substituindo {order_id} pelo ID da order cuja captura total deseja efetuar.
