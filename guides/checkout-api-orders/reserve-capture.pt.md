...
Para realizar a captura do valor total de uma reserva, é necessário enviar uma requisição ao endpoint [/v1/orders/{order_id}/capture :TagComponent{textTag="API"}](/developers/pt/reference/order/online-payments/capture/post) com seu :toolTipComponent[_Access Token_ de teste]{link="/developer/pt" linkText="Chave privada de testes da aplicação criada no Mercado Pago e que é utilizada no _backend_. Você pode acessá-la através de **Suas integrações > Detalhes da aplicação > Testes > Credenciais de teste**."}, substituindo {order_id} pelo ID da order cuja captura total deseja efetuar.
