...
With Mercado Pago's ----[mlb]---- Checkout Transparente,------------ ----[mla, mlm]---- Checkout API ------------ it is also possible to offer payments with **Rapipago** or **Pago Fácil**.
...
| `transaction.payments.payment_method.id` | _Body. String_ | Identifier of the payment method. The options for these payment methods are: <br><br> `rapipago`: for payments with Rapipago. <br><br> `pagofacil`: for payments with Pago Fácil. | Required |
| `transaction.payments.payment_method.type` | _Body. String_ | Type of the payment method. For payments with Rapipago or Pago Fácil, the value must be `ticket`. | Required |
