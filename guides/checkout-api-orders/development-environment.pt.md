...
Para inicializar a biblioteca do Mercado Pago, é necessário utilizar suas :toolTipComponent[credenciais]{link="/developers/pt/docs/checkout-api/more-resources/credentials" linkText="Credenciais" content="Chaves de acesso únicas que usamos para identificar uma integração na sua conta, estando vinculadas à sua aplicação. Para mais informações, acesse o link abaixo."}, chaves únicas que identificam sua integração e estão vinculadas à :toolTipComponent[aplicação]{link="/developers/pt/docs/checkout-api/more-resources/application-details linkText="Detalhes da aplicação" content="Entidade registrada no Mercado Pago que atua como um identificador para gerenciar suas integrações. Para mais informações, acesse o link abaixo."} criada, garantindo que você desenvolva seu projeto contando com as melhores medidas de segurança do Mercado Pago.
...
> NOTE
>
> Se você estiver desenvolvendo para outra pessoa, poderá acessar as credenciais dos aplicativos que você não gerencia. Consulte [Compartilhar credenciais](/developers/pt/docs/checkout-api/more-resources/credentials#bookmark_compartilhar_credenciais#bookmark_compartilhar_credenciais) para mais informações.
