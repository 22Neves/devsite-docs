...
Para continuar, deberás utilizar tu credencial `public key` de producción, a la que podrás acceder ingresando a los Detalles de tu aplicación en Tus integraciones, bajo el título **Producción > Credenciales de producción** en el menú ubicado a la izquierda de la pantalla.
...
También necesitarás utilizar el identificador de la preferencia de pago que obtuviste como respuesta en [Crear y configurar una preferencia de pago](#).
...
instalar la biblioteca SDK MercadoPago.js en el directorio `my-mercadopago-app`. Puedes hacerlo ejecutando el siguiente código.
...
Necesitarás reemplazar el valor `YOUR_PREFERENCE_ID` utilizando el **identificador de la preferencia de pago** que obtuviste como respuesta en [Crear y configurar una preferencia de pago](#).
...
- [Callbacks auxiliares](/developers/es/docs/checkout-pro/checkout-customization/user-interface/auxiliary-callbacks): agrega callbacks que se ejecutarán en momentos específicos del flujo de pago.
:::