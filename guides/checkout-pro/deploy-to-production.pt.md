...
Para fazer isso, acesse [Suas integrações](/developers/panel/app) e, no menu lateral, acesse **Produção > Credenciais de produção**. Lá você encontrará sua `public_key` e `access_token` produtivos, que deverá utilizar no lugar das credenciais da conta de teste.

Para mais informações, consulte nossa documentação de [Credenciais](/developers/pt/docs/checkout-pro/additional-content/your-integrations/credentials).
...
Para saber mais, acesse a documentação [Como medir a qualidade da sua integração](/developers/pt/docs/integration-quality).