...
- **[Alterar](/developers/pt/reference/order/online-payments/update-transaction/patch) e/ou [remover](/developers/pt/reference/order/online-payments/delete-transaction/delete) transação**: a alteração e remoção de transações só pode ser feita no modo manual e permitem mudar informações de pagamento que já tinham sido adicionadas anteriormente à order. São operações que modificam um item dentro de qualquer campo do parâmetro `transactions`.
