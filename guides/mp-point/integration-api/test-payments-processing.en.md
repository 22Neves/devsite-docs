...
1. Send a request to the API ----[mlb]----[Create a payment intent](/developers/en/reference/integrations_api_paymentintent_mlb/_point_integration-api_devices_deviceid_payment-intents/post)------------ ----[mla]----[Create a payment intent](/developers/en/reference/integrations_api/_point_integration-api_devices_deviceid_payment-intents/post)------------ ----[mlm]---- [Crear una intención de pago](/developers/en/reference/point_apis_mlm/_point_integration-api_devices_deviceid_payment-intents/post)------------ using your production credentials. You must assign it to the previously configured Point device by replacing the `deviceId` value in the call path with the corresponding identifier. Also, use a value that allows you to identify this test intent in your system through the `external_reference` field, and make sure to have a minimum amount for the `amount` field.
